...
class MetaMaml(nn.Module):
    def __init__(self, network, n_classes, n_shots=10, n_query=15, lr_inner=0.0001, input_size=(1,28,28), maml_type='maml'):
        super().__init__()
        self.net = network
        self.device = network.device
        self.feature_dim = len(network(torch.randn(input_size).to(self.device)).flatten())
        self.n_classes = n_classes
        self.head = nn.Linear(self.feature_dim, self.n_classes).to(self.device)
        self.device = network.device
        self.n_shots = n_shots
        self.n_query = n_query
        self.lr_inner = lr_inner
        self.net.batch_size = n_classes*n_shots
        self.maml_type = maml_type
        print(
              f'\n########### {maml_type} {n_classes}-way:\t{n_shots}-shot'
              f'\n{network.description}\n',
              f'Mu:  \t{lr_inner}\n',
              )
        self.to(self.device)
    ...
    def forward(self, train_dataset, val_dataset):
        ...
        fast_head = OrderedDict(self.head.named_parameters())
        ...
        for step in range(5):
            ...
            outputs = functional_call(self.head, fast_head, (features,))
            ...
            elif self.maml_type == 'anil':
                # Update head
                fast_head = OrderedDict(
                    (name, param - self.lr_inner * grad)
                    for ((name, param), grad) in zip(fast_head.items(), grads_head)
                )
            elif self.maml_type == 'boil':
                # Update body
                fast_weights = OrderedDict(
                    (name, param - self.lr_inner * grad)
                    for ((name, param), grad) in zip(fast_weights.items(), grads_body)
    ...
    def validation(self, validation_dataset):
        ...
        with torch.no_grad():
            for img, label in test_set:
                ...
                local_label = label_map[label.item()]
                output = functional_call(self.net, fast_weights, (img,))
                output = functional_call(self.head, fast_head, (output,))
